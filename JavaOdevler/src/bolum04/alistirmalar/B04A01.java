package bolum04.alistirmalar;

public class B04A01 {
    public static void main(String[] args) {
        System.out.println("Math.sqrt(4) : " + Math.sqrt(4));
        System.out.println("Math.PI : " + Math.PI);
        System.out.println("Math.sin(2 * Math.PI : " + Math.sin(2 * Math.PI));
        System.out.println("Math.pow(2,2) : " + Math.pow(2, 2));
        System.out.println("Math.log(Math.E) : " + Math.log(Math.E));
        System.out.println("Math.exp(1) : " + Math.exp(1));
        System.out.println("Math.max(2,Math.min(3,4)) : " + Math.max(2, Math.min(3, 4)));
        System.out.println("Math.rint(-2.5) : " + Math.rint(-2.5));
        System.out.println("Math.rint(-2.4) : " + Math.rint(-2.4));
        System.out.println("Math.rint(-2.7) : " + Math.rint(-2.7));
        System.out.println("Math.rint(-3.5) : " + Math.rint(-3.5));
        System.out.println("Math.rint(-4.5) : " + Math.rint(-4.5));
        System.out.println("Math.rint(-5.7) : " + Math.rint(-5.7));
        System.out.println("Math.rint(-6.7) : " + Math.rint(-6.7));
        System.out.println("Math.rint(-6.1) : " + Math.rint(-6.1));
        System.out.println("Math.rint(-7.5) : " + Math.rint(-7.5));
        System.out.println("Math.rint(-8.5) : " + Math.rint(-8.5));
        System.out.println("Math.rint(-3.5f) : " + Math.rint(-3.5f));
        System.out.println("Math.rint(2.1) : " + Math.rint(2.1));
        System.out.println("Math.rint(2.4) : " + Math.rint(2.4));
        System.out.println("Math.rint(2.5) : " + Math.rint(2.5));
        System.out.println("Math.rint(2.7) : " + Math.rint(2.7));
        System.out.println("Math.ceil(-2.5) : " + Math.ceil(-2.5));
        System.out.println("Math.ceil(-2.7) : " + Math.ceil(-2.7));
        System.out.println("Math.ceil(-3.2) : " + Math.ceil(-3.2));
        System.out.println("Math.floor(-2.5) : " + Math.floor(-2.5));
        System.out.println("Math.floor(-4.8) : " + Math.floor(-4.8));
        System.out.println("Math.floor(-5.8) : " + Math.floor(-5.8));
        System.out.println("Math.floor(-6.1) : " + Math.floor(-6.1));
        System.out.println("Math.round(-2.5f) : " + Math.round(-2.5f));
        System.out.println("Math.round(-3.5f) : " + Math.round(-3.5f));
        System.out.println("Math.round(-3.5) : " + Math.round(-3.5));
        System.out.println("Math.round(-3.8) : " + Math.round(-3.8));
        System.out.println("Math.round(-4.8) : " + Math.round(-4.8));
        System.out.println("Math.round(-4.5) : " + Math.round(-4.5));
        System.out.println("Math.round(-3.5) : " + Math.round(-3.5));
        System.out.println("Math.round(4.5) : " + Math.round(4.5));
        System.out.println("Math.round(3.5) : " + Math.round(3.5));

        int round = Math.round(-2.5f);
        double a = 15.5;
        double b = 2.5;

        System.out.println("a: " + Math.rint(a));
        System.out.println("b: " + Math.rint(b));


    }
}
